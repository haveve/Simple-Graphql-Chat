...
const Options = forwardRef<HTMLDivElement, { option: MessageOptionType }>((props, ref) => {
    ...
    return <div className={`${option.show && userId === message?.fromId ? '' : 'd-none'} small chat-info-option d-flex flex-column py-3 position-absolute`} ref={ref}>
        <div className='text-danger' onClick={() => setShowRemove(true)}>{bucket} Remove</div>
        <div className='text-success mt-2' onClick={() => dispatch(setUpdateMessage(message!))}>{updateMessageIcon} Update</div>
