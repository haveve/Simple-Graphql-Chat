...
public class InputAndArgumentNumberValidationRule : IValidationRule, IVariableVisitorProvider
{
    private sealed class FieldVisitor : BaseVariableVisitor
    {
        ...
        {
            AppliedDirective? appliedDirective = field.FindAppliedDirective(ValidationDirectives.NumberRange);
            if (appliedDirective == null)
            {
                return default;
            }

            var obj = appliedDirective.FindArgument("min")?.Value;
            var obj2 = appliedDirective.FindArgument("max")?.Value;
    ...
    {
        AppliedDirective? appliedDirective = provider?.FindAppliedDirective(ValidationDirectives.NumberRange);
        ...
        var min = appliedDirective.FindArgument("min")?.Value;
        var max = appliedDirective.FindArgument("max")?.Value;
